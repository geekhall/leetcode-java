 ...
 * tokens. For each session, the user will receive a new authentication token
 * that will expire timeToLive seconds after the currentTime. If the token is
 * renewed, the expiry time will be extended to expire timeToLive seconds after
 ...
 * countUnexpiredTokens"] [[5], ["aaa", 1], ["aaa", 2], [6], ["bbb", 7], ["aaa",
 * 8], ["bbb", 10], [15]] Output [null, null, null, 1, null, null, null, 0]
 * Explanation AuthenticationManager authenticationManager = new
 * AuthenticationManager(5); // Constructs the AuthenticationManager with
 * timeToLive = 5 seconds. authenticationManager. renew ("aaa", 1); // No token
 * exists with tokenId "aaa" at time 1, so nothing happens.
 * authenticationManager.generate("aaa", 2); // Generates a new token with
 * tokenId "aaa" at time 2. authenticationManager. countUnexpiredTokens (6); //
 * The token with tokenId "aaa" is the only unexpired one at time 6, so return
 * 1. authenticationManager.generate("bbb", 7); // Generates a new token with
 ...
 * renew request is ignored, and nothing happens. authenticationManager. renew
 * ("bbb", 10); // The token with tokenId "bbb" is unexpired at time 10, so the
 * renew request is fulfilled and now the token will expire at time 15.
...
class Solution01797 {
  ...
  public static void test_01797() {
    Solution01797.solution();
  }

  public static void main(String[] args) {
    Solution01797.test_01797();
  }
}
