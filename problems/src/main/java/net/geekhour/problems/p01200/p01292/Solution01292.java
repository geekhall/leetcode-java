package net.geekhour.problems.p01200.p01292;

/**
 * ID: 01292
 * Title: Maximum Side Length of a Square with Sum Less than or Equal to
 * Threshold
 * Difficulty: Medium
 * Description: Given a m x n matrix mat and an integer threshold, return the
 * maximum side-length of a square with a sum less than or equal to threshold or
 * return 0 if there is no such square.
 *
 * Example 1:
 *
 * Input: mat = [[1,1,3,2,4,3,2],[1,1,3,2,4,3,2],[1,1,3,2,4,3,2]], threshold = 4
 * Output: 2 Explanation: The maximum side length of square with sum less than 4
 * is 2 as shown.
 *
 * Example 2:
 *
  ...
  public int maxSideLength(int[][] mat, int threshold) {
    ...
    for (int i = 1; i <= m; i++) {
      for (int j = 1; j <= n; j++) {
        ...
        while (lo < hi) {
          int mid = (lo + hi + 1) / 2;
          if (sum[i + mid - 1][j + mid - 1] - sum[i - 1][j + mid - 1] - sum[i + mid - 1][j - 1]
              + sum[i - 1][j - 1] <= threshold) {
            lo = mid;
          } else {
            hi = mid - 1;
          }
