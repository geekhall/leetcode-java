package net.geekhour.problems.p02000.p02038;
 ...
 * Difficulty: Medium
 * Description: There are n pieces arranged in a line, and each piece is colored
 * either by 'A' or by 'B'. You are given a string colors of length n where
 * colors[i] is the color of the i th piece.
 *
 ...
 * Example 1:
 *
 * Input: colors = "AAABABB" Output: true Explanation: A A ABABB -> AABABB Alice
 * moves first. She removes the second 'A' from the left since that is the only
 * 'A' whose neighbors are both 'A'. Now it's Bob's turn. Bob cannot make a move
 * on his turn since there are no 'B's whose neighbors are both 'B'. Thus, Alice
 * wins, so return true.
 *
 * Example 2:
 *
 * Input: colors = "AA" Output: false Explanation: Alice has her turn first.
 * There are only two 'A's and both are on the edge of the line, so she cannot
 * move on her turn. Thus, Bob wins, so return false.
 *
 * Example 3:
 *
 * Input: colors = "ABBBBBBBAAA" Output: false Explanation: ABBBBBBBA A A ->
 * ABBBBBBBAA Alice moves first. Her only option is to remove the second to last
 * 'A' from the right. ABBBB B BBAA -> ABBBBBBAA Next is Bob's turn. He has many
 * options for which 'B' piece to remove. He can pick any. On Alice's second
 * turn, she has no more pieces that she can remove. Thus, Bob wins, so return
 * false.
 *
 * Constraints:
 *
...
class Solution02038 {
  ...
  public static void test_02038() {
    Solution02038 solution = new Solution02038();
