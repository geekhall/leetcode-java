package net.geekhour.problems.p00900.p00983;

/**
 * ID: 00983
 * Title: Minimum Cost For Tickets
 * Difficulty: Medium
 * Description: You have planned some train traveling one year in advance. The
 * days of the year in which you will travel are given as an integer array days.
...
class Solution00983 {
  ...
  public int mincostTickets(int[] days, int[] costs) {
    ...
    for (int i = 1; i < days.length; i++) {
      ...
      min = Math.min(min, dp[j] + costs[1]);
      j = i - 1;
      while (j >= 0 && days[i] - days[j] < 30) {
        j--;
      }
      min = Math.min(min, dp[j] + costs[2]);
      dp[i] = min;
    }
    return dp[days.length - 1];
  }

  public static void test_00983() {
    Solution00983 solution = new Solution00983();
